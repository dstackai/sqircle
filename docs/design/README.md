...
| [single-squircle-states.md](./single-squircle-states.md) | Reusable one-squircle state recipes for solid, glass, wireframe, text, line, and text + line |
...
- Solid and glass faces can use `off`, `metal`, or `mesh` top-surface effects without moving geometry, and can add clipped surface grain with `grain: true`.
...
- Solid annotations use automatic palette colors; glass annotations use contrast-selected automatic paint; wireframe annotations use wire gradients.
